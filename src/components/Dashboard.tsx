...
const Dashboard = () => {
    ...
    const createResource = async () => {
        try {
            const response = await axios.post('http://localhost:5000/resources', resourceData, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`
                }
            });
            ...
            if (error.response) {
                console.error('Error creating resource:', error.response.status, error.response.data);
            } else {
